...
public class JournalConfig {
    ...
    public static Set<ResourceLocation> blacklistedMobs = new HashSet<>();
    ...
    public static boolean enableTrinketsCompat = true;
    public static boolean enableCuriosCompat = true;
        ...
        try {
            if (! CONFIG_FILE.exists()) {
                ...
                try (FileReader reader = new FileReader(CONFIG_FILE)) {
                    ...
                    enableTrinketsCompat = getOrDefault(data.enable_trinkets_compat, true);
                    enableCuriosCompat = getOrDefault(data.enable_curios_compat, true);
                }
            }
    ...
    public static void addToBlacklist(ResourceLocation mobId) {
        if (blacklistedMobs.add(mobId)) {
            saveBlacklist();
        }
    }

    public static void removeFromBlacklist(ResourceLocation mobId) {
    ...
    private static void saveDefault() {
        ...
        def.enable_trinkets_compat = enableTrinketsCompat;
        def.enable_curios_compat = enableCuriosCompat;
    ...
    private static void migrateOldConfig(ConfigData data) {
        ...
        if (data.enable_trinkets_compat == null) {
            data.enable_trinkets_compat = true;
            changed = true;
        }
        if (data.enable_curios_compat == null) {
            data.enable_curios_compat = true;
            changed = true;
        }
    ...
    private static String getCurrentModVersion() {
        try {
            return Platform.getMod(Journal.MOD_ID).getVersion();
        } catch (Exception e) {
            return "unknown";
        }
    }
    ...
    public static void setMobRenderConfig(ResourceLocation mobId, boolean isGridMode, float scale, float xOffset, float yOffset, float speed, float smoothing, AnimationMode animationMode) {
    ...
    public static void deleteMobRenderConfig(ResourceLocation mobId, boolean isGridMode) {
        String key = mobId.toString();
        Map<String, MobRenderConfig> configs = isGridMode ? gridRenderConfigs :  detailRenderConfigs;
        configs.remove(key);
    }
    ...
    public static MobRenderConfig getMobRenderConfig(ResourceLocation mobId, boolean isGridMode) {
        String key = mobId.toString();
        Map<String, MobRenderConfig> configs = isGridMode ? gridRenderConfigs : detailRenderConfigs;
    ...
    private static class ConfigData {
        ...
        public Boolean enable_trinkets_compat;
        public Boolean enable_curios_compat;
    }
}
