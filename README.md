...
```json
"Tame with [bones](hover:item:minecraft:bone), or feed a cat any [fish](hover:tag_grid:minecraft:fishes)."
```
...
Vanilla's per-mob food tags (`#minecraft:wolf_food` and friends) only exist in 1.21+, so these bundled descriptions reference items directly instead. `tag:` and `tag_grid:` still work here — point them at a tag this version has, such as `#minecraft:fishes` or `#minecraft:flowers`.
